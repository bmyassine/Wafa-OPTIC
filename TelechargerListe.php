...
$listereclamatio=$reclamatioC->afficherreclamatios();
...
<table border="1" style="border: 2px solid black; border-collapse: collapse;" id="content">
    <caption style="margin: 5px;"><strong>Liste Des reclamations </strong></caption>
    <tr>
        <td><strong>Date de la reclamation</strong></td>
        <td><strong>Numero tel</strong></td>
        <td><strong>Objet</strong></td>
        <td><strong>Details reclamation</strong></td>
        <td><strong>Etat reclamation</strong></td>
    </tr>

<?PHP
foreach($listereclamatio as $row){
	?>
	<tr>
	<td><?PHP echo $row['DATE_reclamatio']; ?></td>
>
	<td><?PHP echo $row['NUM_R']; ?></td>
	<td><?PHP echo $row['OBJET_R']; ?></td>
	<td><?PHP echo $row['DETAILS_R']; ?></td>
	<td><?PHP echo $row['ETAT_R']; ?></td>
    </tr>
	<?PHP
}
?>
