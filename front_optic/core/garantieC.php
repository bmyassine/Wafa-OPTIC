<?PHP
 class garantieC {
    ...
    {
        $sql="SElECT * From garantie s inner join client u on s.USER_ID= u.ID";
        //$sql="SElECT d.ID_garantie,d.NOW_garantie,d.DATE_garantie,d.OBJET_garantie,d.ETAT_garantie,d.USER_ID  FROM garantie d ";
        $db = config::getConnexion();
        try{
            $liste=$db->query($sql);
            return $liste;
...
function chercherClient($chercher,$champs,$ordre) {
    $db=config::getConnexion();
    $sql="SElECT * From garantie s inner join client u on s.USER_ID= u.ID where $champs like '%$chercher%' ORDER BY $champs $ordre ";
    try
    {
        $liste=$db->query($sql);
        return $liste;
